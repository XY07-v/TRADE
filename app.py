...
visitas_col = db['Visitas'] # Se crea automáticamente si no existe

# USUARIOS (Cédula: Nombre)
USUARIOS = {"12345678": "Andrés Vanegas", "87654321": "Admin Power", "10102020": "Soporte Técnico"}
...
def get_puntos():
    return jsonify(list(puntos_col.find({}, {"_id":0})))

@app.route('/api/guardar_punto', methods=['POST'])
def guardar_punto():
    data = request.json
    if puntos_col.find_one({"BMB": data['BMB']}): return jsonify({"status":"err", "msg":"BMB Duplicado"}), 400
    if puntos_col.find_one({"Nombre de punto": data['Nombre de punto']}): return jsonify({"status":"err", "msg":"Nombre Duplicado"}), 400
...
def guardar_visita():
    data = request.json
    # Calcular distancia entre el punto original y donde está parado el usuario ahora
